 ...
 *
 * Switch IMU quaternion packing is derived from MissionControl's
...
// Switch2 が TOGGLE_IMU=0x02~0x05 で要求する quaternion mode 用の換算定数。
...
// quaternion packet 内の accel ベクトルは wire layout が y,x,z 順。
// C++ 側のフィールド名もこの順にしておき、memcpy 時のバイト配置を明示する。
typedef struct __attribute((packed, aligned(1))) {
  int16_t y;
  int16_t x;
  int16_t z;
} SwitchIMUQuaternionVector;

// Switch2 の 36 byte IMU payload。
// quaternion は最大成分を省いた 3 成分として bitfield に格納し、accel は先頭の accel0 を使う。
typedef struct __attribute((packed, aligned(1))) {
  SwitchIMUQuaternionVector accel0;
  uint32_t mode : 2;
  uint32_t maxIndex : 2;
...
} SwitchIMUQuaternionData;

static_assert(sizeof(SwitchIMUQuaternionData) == 36, "Switch IMU quaternion payload must be 36 bytes");
...
// 正規化済み quaternion を Switch2 の形式に圧縮する。
// 4成分のうち絶対値が最大の成分を maxIndex として省略し、残り3成分だけを packet に詰める。
void pack2Quaternion(SwitchIMUQuaternionData &quatData, const float quat[4]) {
  ...
  quatData.maxIndex = maxIndex;
  quatData.lastSample0 = static_cast<uint32_t>(quaternionComponents[0] >> 10);
  quatData.lastSample1Low = static_cast<uint32_t>((quaternionComponents[1] >> 10) & 0x7F);
  quatData.lastSample1High = static_cast<uint16_t>(((quaternionComponents[1] >> 10) & 0x1FFF80) >> 7);
  quatData.lastSample2Low = static_cast<uint16_t>((quaternionComponents[2] >> 10) & 0x03);
  quatData.lastSample2High = static_cast<uint32_t>(((quaternionComponents[2] >> 10) & 0x1FFFFC) >> 2);
}
...
bool SwitchProDriver::process(Gamepad *gamepad) {
  ...
  switchReport.inputs.leftStick.setY(leftMaxY - std::min(std::max(scaleLeftStickY, leftMinY), leftMaxY));
  switchReport.inputs.rightStick.setX(std::min(std::max(scaleRightStickX, rightMinX), rightMaxX));
  switchReport.inputs.rightStick.setY(rightMaxY - std::min(std::max(scaleRightStickY, rightMinY), rightMaxY));
...
// quaternion mode の姿勢推定状態と timestamp を初期状態へ戻す。
void SwitchProDriver::resetIMUState() {
  // quaternion mode 開始時や IMU 無効化後は identity quaternion から再開する。
  ...
  imuTimestampRemainderUs = 0;
  imuTimestamp = 0;
}

// TOGGLE_IMU で要求された mode に応じて raw IMU と quaternion mode を振り分ける。
void SwitchProDriver::updateIMUData(Gamepad *gamepad) {
  ...
  case 0x01:
    ...
    break;
  // なぜか mode 0x02～0x05 まで同じ quaternion mode を要求してくるため、まとめて処理する。
  case 0x02:
  case 0x03:
  case 0x04:
  case 0x05:
    updateQuaternionIMUData(gamepad);
    break;
  default:
    memset(switchReport.imuData, 0x00, sizeof(switchReport.imuData));
    imuQuaternionLastTimeUs = 0;
...
void SwitchProDriver::updateRawIMUData(Gamepad *gamepad) {
  ...
  const uint8_t sampleCount = sensors.imuSampleCount > GAMEPAD_AUX_MAX_IMU_SAMPLES ? GAMEPAD_AUX_MAX_IMU_SAMPLES : sensors.imuSampleCount;
  if (sampleCount == 0) {
  ...
  for (uint8_t sample = 0; sample < GAMEPAD_AUX_MAX_IMU_SAMPLES; sample++) {
    const uint8_t sampleIndex = sample < sampleCount ? GAMEPAD_AUX_MAX_IMU_SAMPLES - 1 - sample : firstSample;
    writeIMUSample(&switchReport.imuData[sample * 12], sensors.imuSamples[sampleIndex]);
  }
}

// Switch2 が要求する quaternion payload を生成する。
void SwitchProDriver::updateQuaternionIMUData(Gamepad *gamepad) {
  // addon 側 FIFO の最新サンプルを取り出し、quaternion mode 用の内部座標 frame に変換する。
  ...
  quaterniomIMUFrame frame = toQuaternionFrame(sample);
  ...
  updateAttitude(frame, deltaUs);

  // quaternion mode packet 内の timestamp は ms 単位の 11 bit 値として扱う。
  // report 間隔は us で測るため、1000 us 未満の端数を次回へ繰り越す。
  imuTimestampRemainderUs += deltaUs;
  if (imuTimestampRemainderUs >= 1000) {
    uint64_t wholeMs = imuTimestampRemainderUs / 1000;
    imuTimestampRemainderUs %= 1000;
    imuTimestamp = static_cast<uint16_t>((imuTimestamp + wholeMs) % 0x7FF);
  }

  SwitchIMUQuaternionData quatData = {};
  float quat[4] = {
      imuQuaternionX,
      imuQuaternionY,
      imuQuaternionZ,
  ...
  // quaternion mode の accel0 は wire layout が { y, x, z }。
  // 実機確認の結果、Switch 側の accel.x 解釈は姿勢推定 frame の X と符号が逆だったため、
  // packet へ送る x 成分だけを反転する。
  quatData.accel0 = {
      frame.accelY,
  ...
  // quaternion と accel を 36 byte の payload に詰め、SwitchProReport::imuData へコピーする。
  quatData.mode = 2;
  pack2Quaternion(quatData, quat);
  quatData.timestampStartLow = imuTimestamp & 0x01;
  quatData.timestampStartHigh = (imuTimestamp >> 1) & 0x03FF;
  quatData.timestampCount = 3;

  memcpy(switchReport.imuData, &quatData, sizeof(quatData));
}

// BoardConfig 適用後の IMU sample を、quaternion mode の姿勢推定で使う signed frame に変換する。
SwitchProDriver::quaterniomIMUFrame SwitchProDriver::toQuaternionFrame(const GamepadAuxIMUSample &sample) const {
  // GamepadAuxIMUSample は BoardConfig と WebConfig の軸設定を反映済みの値。
  // ここでは quaternion 姿勢推定で使う signed int16 値に戻し、raw 経路とは独立した frame として扱う。
...
// gyro 積分で内部 quaternion を更新する。
void SwitchProDriver::updateAttitude(const quaterniomIMUFrame &frame, uint64_t deltaUs) {
  if (deltaUs == 0) {
    return;
  }
  ...
  // q と -q は同じ姿勢を表すが、符号が急に反転すると圧縮後の値が跳ねる。
